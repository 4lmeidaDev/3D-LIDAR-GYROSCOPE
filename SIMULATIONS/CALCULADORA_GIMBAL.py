...
class LidarSimulatorUI:
    def __init__(self, root):
        self.root = root
        self.root.title("LIDAR 3D Analyzer v15.0 - Motor de Renderização em Tempo Real")
        self.root.geometry("1400x950")
        
        self.paned = ttk.PanedWindow(root, orient=tk.HORIZONTAL)
        self.paned.pack(fill=tk.BOTH, expand=True)
        
        # --- SISTEMA DE SCROLL NO PAINEL ESQUERDO ---
        ...
        chk_corte = ttk.Checkbutton(
            rot_group, 
            text="Ocultar Metade Traseira", 
            variable=self.var_cortar_metade,
            command=self.atualizar_grafico 
        )
        chk_corte.pack(fill=tk.X, pady=(10, 0))

        # ---> NOVA TICKBOX DE ANIMAÇÃO <---
        self.var_animar = tk.BooleanVar(value=False)
        chk_animar = ttk.Checkbutton(
            rot_group, 
            text="VER SIMULAÇÃO DE PONTOS EM TEMPO REAL", 
            variable=self.var_animar,
            command=self.on_toggle_animacao 
        )
    ...
    def create_input_group(self, key, label, default, p_min, p_max, step):
        ...
        if key == "gimbal":
            ...
            chk_otimizar = ttk.Checkbutton(
                group, 
                text="Usar valor otimizado", 
                variable=self.var_otimizar_gimbal,
                command=self.on_toggle_otimizar
            )
    ...
    def on_toggle_animacao(self):
        # Quando o utilizador clica na tickbox, recomeça a animação
        self.anim_index = 0
    ...
    # --- O CÉREBRO MATEMÁTICO (Apenas faz as contas) ---
    def atualizar_grafico(self):
        try:
            params = {k: float(self.entries[k].get()) for k in self.entries}
            
            raio_buraco_m = calcular_maior_buraco(
                params["fov"], params["rate"], params["scan"], params["gimbal"], 
                params["dist"], params["dur"]
            )
            ...
            x, y, z = simular_rastreio(
                params["fov"], params["rate"], params["scan"], params["gimbal"], 
                r=params["dist"], duracao=params["dur"]
            )
            tempo_decorrido = np.arange(len(x))

            # Aplica o Clipping Plane se estiver ativo
            if self.var_cortar_metade.get():
                e = int(round(self.ax.elev))
                a = int(round(self.ax.azim)) % 360
            ...
            # Guarda os arrays completos na memória da classe
            self.last_x, self.last_y, self.last_z = x, y, z
            self.last_c = tempo_decorrido
            self.last_dist = params["dist"]
            self.last_grid = params["grid"]

            # Sempre que os parâmetros mudam, o frame da animação volta a zero
            self.anim_index = 0 
            
            # Manda a placa gráfica desenhar a nova realidade
            self.desenhar_grafico()
            
        except Exception as e:
            pass

    # --- O MOTOR DE RENDERIZAÇÃO (Desenha e Anima) ---
    def desenhar_grafico(self):
        ...
        # 1. Guarda a orientação atual da câmara e bloqueia o movimento indesejado
        ...
        if hasattr(self, 'var_animar') and self.var_animar.get():
            ...
            # O "passo" dinâmico: garante que varre o gráfico todo em ~40 frames independentemente do nº de pontos
            self.anim_step = max(1, len(self.last_x) // 40)
            self.anim_index += self.anim_step

            # Dá a ordem para o gráfico desenhar apenas do ponto 0 até ao índice atual
            x = self.last_x[:self.anim_index]
            y = self.last_y[:self.anim_index]
            z = self.last_z[:self.anim_index]
            c = self.last_c[:self.anim_index]

            # Quando chega ao fim, recomeça do zero (Efeito de varrimento contínuo)
            if self.anim_index > len(self.last_x):
                self.anim_index = 0

            # Limpa o relógio anterior e agenda o próximo frame (40ms = ~25 fps)
            if hasattr(self, 'anim_job') and self.anim_job:
                self.root.after_cancel(self.anim_job)
            self.anim_job = self.root.after(40, self.desenhar_grafico)
            
        else:
            # Se a tickbox for desligada, para a animação imediatamente e desenha o gráfico completo
            if hasattr(self, 'anim_job') and self.anim_job:
                self.root.after_cancel(self.anim_job)
                self.anim_job = None

        # 3. Limpeza Cirúrgica do Ecrã
        ...
        # Evita tentar desenhar um array vazio no primeiro milissegundo da animação
        if len(x) > 0:
            self.ax.scatter(x, y, z, c=c, cmap='plasma', s=12, alpha=0.7)
        
        # 5. Restauro do Ambiente Trancado (A Caixa, Limites e Grelha)
        lim = dist * 1.2
