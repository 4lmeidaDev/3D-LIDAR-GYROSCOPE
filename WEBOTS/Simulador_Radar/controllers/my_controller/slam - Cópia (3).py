...
REORTHO_EVERY = 50
MIN_BUF_PTS   = 40
ACCEL_DEAD    = 0.15   # m/s² — threshold da dead-zone. 0.05 era demasiado baixo:
                       # ruído típico do accel Webots é ~0.05-0.10 m/s²,
                       # portanto com 0.05 nunca entrava na dead-zone e acumulava drift.
...
MAX_STEP_M      = 5.0
MIN_MOVE_M      = 0.03
ICP_MAX_ROT_DEG = 15.0   # rejeitar ICP se correcção de rotação > N graus

# Correcção de yaw residual após ICP (só activa sem InertialUnit)
ICP_MAX_YAW_DEG = 20.0   # rejeitar correcção de yaw acima deste ângulo
ICP_MIN_YAW_DEG = 0.3    # ignorar ruído abaixo deste ângulo

# Seta de orientação na visualização
ARROW_LEN = 0.5           # comprimento em metros

# Sliding window: referência ICP usa só as últimas N iterações do mapa
...
def _integrate_gyro(R, omega, dt):
    """
    Rodrigues: integra omega [rad/s] do gyro (no referencial do CORPO) durante dt.
    omega está no corpo → dR é rotação incremental no corpo.
    R_new = R @ dR  acumula corpo→mundo correctamente.
    """
...
def _update_pose(R, v, t, a_body, dt):
    ...
    ATENÇÃO: a dead-zone só actua quando a_world≈0 E v é baixa.
    Se a_world≈0 mas v é alta → velocidade constante → manter v intacta.
    Zerar v quando a_world≈0 e v alta era o bug: parava o robot a meio caminho.
    """
    a_world = R.T @ a_body + GRAVITY
    a_norm  = float(np.linalg.norm(a_world))
    speed   = float(np.linalg.norm(v))

    if a_norm < ACCEL_DEAD and speed < 0.10:
        # Parado de verdade: aceleração nula E velocidade baixa → zerar v
        v_new = np.zeros(3, dtype=np.float64)
    else:
        # Em movimento (qualquer velocidade) ou a acelerar → integrar normalmente
        # Quando a_world≈0 e speed>0.10: velocidade constante, não tocar em v
        v_new = v + a_world * dt
    t_new = t + v_new * dt
    return v_new, t_new
...
# ──────────────────────────────────────────────────────────────
# CORRECÇÃO DE YAW POR ICP
#
# Após o ICP de translação+rotação, pode restar um erro de yaw acumulado
# pelo gyro. Esta função estima esse resíduo via SVD 2D em XY:
#   H   = Σ (src_i - μ_src)_xy^T × (tgt_i - μ_tgt)_xy   [2×2]
#   R2  = Vt.T @ U.T
#   yaw = atan2(R2[1,0], R2[0,0])
# Só aplicado se ICP_MIN_YAW_DEG < |yaw| < ICP_MAX_YAW_DEG.
# Desactivado automaticamente se InertialUnit estiver disponível.
# ──────────────────────────────────────────────────────────────
def _estimar_yaw_corr(src_alinhado, tgt):
    """
    src_alinhado : buffer já corrigido pela translação/rotação ICP (float32, Nx3)
    tgt          : referência do mapa (float64, Mx3)
    Retorna (yaw_deg, R_z_3x3) ou (0.0, eye(3)) se inválido/fora do intervalo.
    """
    if not _HAS_KDTREE:
        return 0.0, np.eye(3)

    s = _voxel(src_alinhado.astype(np.float32), VOXEL_ICP).astype(np.float64)
    t = _voxel(tgt.astype(np.float32),          VOXEL_ICP).astype(np.float64)
    if len(s) > MAX_ICP_PTS: s = s[np.random.choice(len(s), MAX_ICP_PTS, replace=False)]
    if len(t) > MAX_ICP_PTS: t = t[np.random.choice(len(t), MAX_ICP_PTS, replace=False)]
    ...
    tree        = _KDTree(t)
    dists, idxs = tree.query(s, k=1, workers=1)
    mask        = dists <= min(float(np.percentile(dists, 80)), ICP_MAX_DIST)
    if mask.sum() < 20:
        return 0.0, np.eye(3)

    # SVD 2D sobre XY — estima rotação em torno de Z (yaw) apenas
    src_xy = s[mask][:, :2]
    tgt_xy = t[idxs[mask]][:, :2]
    mu_s   = src_xy.mean(0)
    mu_t   = tgt_xy.mean(0)
    H      = (src_xy - mu_s).T @ (tgt_xy - mu_t)
    U, _, Vt = np.linalg.svd(H)
    R2 = Vt.T @ U.T
    if np.linalg.det(R2) < 0:
        Vt[-1] *= -1; R2 = Vt.T @ U.T

    yaw_rad = math.atan2(R2[1, 0], R2[0, 0])
    yaw_deg = math.degrees(yaw_rad)

    if abs(yaw_deg) < ICP_MIN_YAW_DEG or abs(yaw_deg) > ICP_MAX_YAW_DEG:
        return 0.0, np.eye(3)

    c, s_ = math.cos(yaw_rad), math.sin(yaw_rad)
    R_z   = np.array([[ c, -s_, 0.0],
                      [s_,  c,  0.0],
...
def _vispy_thread():
    ...
    # Seta de orientação: linha ciana + ponto na ponta
    ...
    def _update(ev):
        try:
            mapa, buf, traj, pos_lidar, modo, lc_pts, heading_vec, yaw_deg = _q.get_nowait()
        except queue.Empty:
            return
        if len(mapa) > 1:
        ...
        robot_vis.set_data(np.array([pos_lidar], np.float32),
                           ...
                           size=22, symbol='disc')
        # Keyframes de loop closure a magenta
        if len(lc_pts) > 0:
            lc_vis.set_data(np.array(lc_pts, np.float32),
                            edge_color=None, face_color=(1., 0., 1., 0.9), size=14, symbol='star')
        # Seta ciana de orientação — mostra para onde o LIDAR aponta
        tip = (pos_lidar + heading_vec).astype(np.float32)
        arrow_vis.set_data(np.array([pos_lidar, tip], np.float32))
...
while robot.step(timestep) != -1:
    ...
    if len(pts_body) > 0:
        # R_imu representa orientação corpo→mundo (coluna i = eixo i do corpo no mundo).
        # Para transformar pontos do corpo para o mundo: pts_world = R_imu @ pts_body + t_imu
        # Se os pontos rodarem com o robot em vez de ficarem fixos no mundo,
        # a convenção de R_imu está invertida → usar R_imu.T
        # (R_imu.T = R_mundo_para_corpo → inverso = corpo_para_mundo quando R é ortogonal)
        pts_w = ((R_imu.T @ pts_body.T).T + t_imu).astype(np.float32)
    ...
    if t_agora - last_icp_time >= T_ICP and len(_buf) >= MIN_BUF_PTS:
        ...
        if not _bootstrapped:
            ...
            if ok:
                ...
                t_imu = t_imu + t_corr
                # Aplicar correcção de rotação ao R_imu
                ...
                # Correcção de yaw residual — só sem InertialUnit
                # O ICP 6-DOF já corrigiu a maior parte; esta segunda passagem
                # apanha o erro de yaw que o gyro acumula entre ciclos ICP.
                yaw_corr_deg = 0.0
                if not _imu:
                    buf_alinhado = (_buf + t_corr).astype(np.float32)
                    yaw_corr_deg, R_z = _estimar_yaw_corr(buf_alinhado, mapa_ref)
                    if abs(yaw_corr_deg) > ICP_MIN_YAW_DEG:
                        R_imu = _reortho(R_z @ R_imu)
                ...
                yaw_str = f"  Δyaw={yaw_corr_deg:+.1f}°" if abs(yaw_corr_deg) > ICP_MIN_YAW_DEG else ""
                print(f"[LIO] #{n_icp:3d}  rmse={rmse:.3f} {'✓' if conv else '~'}  "
                      f"ΔR={rot_deg:.1f}°{yaw_str}  "
    ...
    if t_agora - last_plot_time >= PLOT_INTERVAL:
        ...
        lidar_pos = (R_imu.T @ np.array([0., 0., Z_TORRE]) + t_imu).astype(np.float32)
        # Seta: primeira coluna de R_imu.T = eixo X do corpo no mundo
        heading_vec   = (R_imu.T[:, 0] * ARROW_LEN).astype(np.float32)
        yaw_atual_deg = math.degrees(math.atan2(R_imu.T[1, 0], R_imu.T[0, 0]))
